...
;
; PURPOSE:
;   The SetMapParamet class is a helper class for objects with
;   map projection data.
;
...
;----------------------------------------------------------------------------
function projectionparameter::Init, _REF_EXTRA=_extra

  compile_opt idl2, hidden
  if (self->IDLITCOMPONENT::INIT(_Extra=extra) ne 1) then RETURN, 0
  self->SETPROPERTYATTRIBUTE, 'description', /hide
  self->SETPROPERTYATTRIBUTE, 'name', /hide
  self->projectionparameter::_RegisterProperties
  self->_UpdateProjection
  
  if (N_ELEMENTS(_extra) gt 0) then $
    self->projectionparameter::SetProperty, _EXTRA=_extra
...
;----------------------------------------------------------------------------
pro projectionparameter::_RegisterProperties, $
...
;
pro projectionparameter::Restore
  compile_opt idl2, hidden
...
;----------------------------------------------------------------------------
function projectionparameter::_GetMapStructure
  ...
  self->projectionparameter::GetProperty, ZONE=zone
...
;
pro projectionparameter::_UpdateUTMLongitude
...
;
pro projectionparameter::_UpdateStatePlane
...
;
pro projectionparameter::_UpdateEllipsoid
...
;
pro projectionparameter::_UpdateLimit
...
;----------------------------------------------------------------------------
pro projectionparameter::_UpdateProjection
...
;-------------------------------------------------------------------------
function projectionparameter::GetPreview
...
;----------------------------------------------------------------------------
pro projectionparameter::GetProperty, $
...
;----------------------------------------------------------------------------
pro projectionparameter::SetProperty, $
    CENTER_LONGITUDE=centerLongitude, $
    CENTER_LATITUDE=centerLatitude, $
  ...
  if (N_ELEMENTS(strEllipsoid) eq 1) then begin
    ...
    if (match ge 0) then begin
      self->projectionparameter::SetProperty, IELLIPSOID=match
    endif else begin
      self->ErrorMessage, SEVERITY=1, $
  ...
  if (N_ELEMENTS(semiMajor) eq 1 && self._semiMajor ne semiMajor) then begin
    ...
    self->projectionparameter::GetProperty, IELLIPSOID=ell
    if (ell ne 0) then $
      self->projectionparameter::SetProperty, IELLIPSOID=0
  endif
  ...
  if (N_ELEMENTS(gctpzone) eq 1) then begin
    ...
    if (self._projection eq 'State Plane') then begin
      ...
      if (ABS(gctpzone) le 60 && gctpzone ne 0) then begin
        ...
        self->projectionparameter::SetProperty, UTM_ZONE=ABS(gctpzone)
      endif else begin
        badZone = 1
      endelse
    endelse
  ...
  if N_ELEMENTS(_extra) gt 0 then self->IDLITCOMPONENT::SETPROPERTY, _Extra = _extra
end
...
;
; PURPOSE:
;    Defines the object structure for an projectionparameter object.
;
;-
pro  projectionparameter__Define
